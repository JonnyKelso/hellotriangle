...
namespace Graphics
{
    ...
    class TextureBinder
    {
        ...
        int m_gl_texture{};
    public:
        TextureBinder(const int gl_texture, std::filesystem::path path, const bool verticalFlip = false)
        : m_gl_texture{gl_texture}
        {
            glGenTextures(1, &m_Texture);
            bind();

            load(path, verticalFlip);
        }
        ...
        void load(std::filesystem::path path, const bool verticalFlip = false)
        {
            if(path.empty())
            {
                return;
            ...
            if (int width, height, nrChannels; unsigned char* data = stbi_load(path.c_str(), &width, &height, &nrChannels, 0))
            {
                GLenum format{};
                switch (nrChannels)
                {
